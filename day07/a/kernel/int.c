...
/* 来自PS/2鼠标的中断 */
{
	...
	putfonts8_asc(binfo->vram, binfo->scrnx, 0, 0, COL8_FFFFFF, "INT 06");
	for (;;) {
		io_hlt();
	}
}
...
/* 来自PS/2鼠标的中断 */
{
	struct BOOTINFO *binfo = (struct BOOTINFO *) ADR_BOOTINFO;
	boxfill8(binfo->vram, binfo->scrnx, COL8_000000, 0, 0, 32 * 8 - 1, 15);
	putfonts8_asc(binfo->vram, binfo->scrnx, 0, 0, COL8_FFFFFF, "INT 08");
	for (;;) {
		io_hlt();
	}
}
...
/* 来自PS/2鼠标的中断 */
{
	struct BOOTINFO *binfo = (struct BOOTINFO *) ADR_BOOTINFO;
	boxfill8(binfo->vram, binfo->scrnx, COL8_000000, 0, 0, 32 * 8 - 1, 15);
	putfonts8_asc(binfo->vram, binfo->scrnx, 0, 0, COL8_FFFFFF, "INT 0C");
	for (;;) {
		io_hlt();
	}
}
...
{
	struct BOOTINFO *binfo = (struct BOOTINFO *) ADR_BOOTINFO;
	unsigned char data,s[40];
	io_out8(PIC0_OCW2,0x61);  //通知PIC "IRQ-01已经受理完毕"
	data=io_in8(PORT_KEYDAT);
	// sprintf(s,"%02X",data);
	sprintf(s,"INT Keyboard value:%02X",data);
