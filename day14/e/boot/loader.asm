...
; LABEL_DESC_FLAT_C:		Descriptor             0,              0fffffh, DA_CR  | DA_32			; 0 ~ 4G
...
LABEL_START:			; <--- 从这里开始 *************
	...
	;检查VBE版本,如果小于V2.0, 跳到scrn320.
	mov ax,[es:di+4]
	cmp ax,0x0200
	jb	scrn320

	;取得画面模式信息,如果是0x004f之外的值，制定模式不可用
	mov cx,VBEMODE
	mov	ax,0x4f01
	int 0x10
	cmp ax,0x004f
...
.MemChkOK:
	...
	mov	word [wSectorNo], SectorNumOfRootDirStart	
	xor	ah, ah	; ┓
	xor	dl, dl	; ┣ 软驱复位
	int	13h	; ┛
...
LABEL_FILENAME_FOUND:			; 找到 KERNEL.BIN 后便来到这里继续
	...
	add	cx, ax
	add	cx, SectorBalance	; 这时 cl 里面是 LOADER.BIN 的起始扇区号 (从 0 开始数的序号)
	mov	ax, BaseOfKernelFile
	mov	es, ax			; es <- BaseOfKernelFile
	mov	bx, OffsetOfKernelFile	; bx <- OffsetOfKernelFile	于是, es:bx = BaseOfKernelFile:OffsetOfKernelFile = BaseOfKernelFile * 10h + OffsetOfKernelFile
...
;字符串
;----------------------------------------------------------------------------
KernelFileName		db	"HARIBOTEBIN", 0	; KERNEL.BIN 之文件名
...
LABEL_EVEN:;偶数
	...
	add	ax, SectorNumOfFAT1Start	; 此句执行之后的 ax 就是 FATEntry 所在的扇区号
	mov	cl, 2
